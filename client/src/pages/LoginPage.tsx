import { useState } from "react";
import { Eye, EyeOff, AlertCircle, Smartphone, Lock } from "lucide-react";
import { useAuth } from "../hooks/useAuth";
import { useI18n } from "../hooks/useI18n";

const LOGO_LG = "data:image/png;base64,";

type Step = "credentials" | "mfa";

export function LoginPage() {
  const { t } = useI18n();
  const { login, completeMfaLogin } = useAuth();
  const [email, setEmail]         = useState("");
  const [password, setPassword]   = useState("");
  const [showPwd, setShowPwd]     = useState(false);
  const [step, setStep]           = useState<Step>("credentials");
  const [mfaUserId, setMfaUserId] = useState<number | null>(null);
  const [mfaCode, setMfaCode]     = useState("");
  const [error, setError]         = useState<string | null>(null);
  const [loading, setLoading]     = useState(false);

  async function handleCredentials(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    setLoading(true);
    try {
      const result = await login(email, password);
      if (result.mfaRequired && result.userId) { setMfaUserId(result.userId); setStep("mfa"); }
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "Identifiants invalides");
    } finally { setLoading(false); }
  }

  async function handleMfa(e: React.FormEvent) {
    e.preventDefault();
    if (!mfaUserId) return;
    setError(null);
    setLoading(true);
    try { await completeMfaLogin(mfaUserId, mfaCode); }
    catch (err: unknown) { setError(err instanceof Error ? err.message : "Code invalide"); setMfaCode(""); }
    finally { setLoading(false); }
  }

  const inputStyle: React.CSSProperties = {
    width: "100%",
    background: "var(--wr-surface)",
    border: "1px solid var(--wr-border2)",
    borderRadius: 8,
    padding: "10px 14px",
    fontSize: 13,
    color: "var(--wr-text-1)",
    fontFamily: "inherit",
    outline: "none",
    transition: "border-color 0.15s",
  };

  return (
    <div style={{ minHeight: "100vh", display: "flex", background: "var(--wr-bg)" }}>

      {/* ── Panneau gauche — Branding ──────────────────────────────────────── */}
      <div style={{
        width: "45%",
        background: "linear-gradient(160deg, #0E1628 0%, #111F35 60%, #0B1525 100%)",
        borderRight: "1px solid rgba(255,255,255,0.05)",
        display: "flex",
        flexDirection: "column",
        justifyContent: "space-between",
        padding: "48px",
        position: "relative",
        overflow: "hidden",
      }}
      className="hidden lg:flex"
      >
        {/* Grille subtile */}
        <div style={{
          position: "absolute", inset: 0, opacity: 0.025,
          backgroundImage: "linear-gradient(rgba(212,175,55,1) 1px, transparent 1px), linear-gradient(90deg, rgba(212,175,55,1) 1px, transparent 1px)",
          backgroundSize: "52px 52px",
          pointerEvents: "none",
        }} />

        {/* Halo central */}
        <div style={{
          position: "absolute",
          top: "35%", left: "50%",
          transform: "translate(-50%,-50%)",
          width: 400, height: 400,
          borderRadius: "50%",
          background: "radial-gradient(circle, rgba(212,175,55,0.06) 0%, transparent 65%)",
          pointerEvents: "none",
        }} />

        {/* Logo */}
        <div style={{ position: "relative", zIndex: 1 }}>
          <img
            src={LOGO_LG}
            alt="WatchReg AML/KYC Solutions"
            style={{ height: 72, width: "auto", objectFit: "contain", objectPosition: "left center" }}
          />
        </div>

        {/* Contenu central */}
        <div style={{ position: "relative", zIndex: 1 }}>
          {/* Badge version */}
          <div style={{
            display: "inline-flex", alignItems: "center", gap: 6,
            background: "rgba(212,175,55,0.06)",
            border: "1px solid rgba(212,175,55,0.15)",
            borderRadius: 20,
            padding: "5px 12px",
            marginBottom: 24,
          }}>
            <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#34D399", boxShadow: "0 0 5px rgba(52,211,153,0.6)" }} />
            <span style={{ fontSize: 10, fontFamily: "monospace", letterSpacing: "0.15em", textTransform: "uppercase", color: "rgba(212,175,55,0.65)" }}>
              Plateforme v2.0 · 2026
            </span>
          </div>

          <h1 style={{
            fontSize: 32,
            fontWeight: 400,
            fontFamily: "'Playfair Display', Georgia, serif",
            color: "var(--wr-sidebar-text-1)",
            lineHeight: 1.25,
            letterSpacing: "-0.5px",
            marginBottom: 16,
          }}>
            Conformité AML/KYC<br />
            <span style={{
              background: "linear-gradient(135deg, #D4AF37 0%, #B8953D 60%, #D4AF37 100%)",
              WebkitBackgroundClip: "text",
              WebkitTextFillColor: "transparent",
            }}>
              en temps réel
            </span>
          </h1>

          <p style={{
            fontSize: 13,
            color: "rgba(180,196,216,0.55)",
            lineHeight: 1.8,
            maxWidth: 340,
            fontFamily: "monospace",
            marginBottom: 32,
          }}>
            Détection AML · Screening OFAC/EU/ONU/UK · Moteur de règles no-code · Rapports GoAML · Conformité AMLD6 · BAM Maroc
          </p>

          {/* Stats */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
            {[
              { val: "< 500ms", lbl: "Analyse tx" },
              { val: "4 listes", lbl: "Sanctions" },
              { val: "AMLD6",   lbl: "Conformité" },
            ].map(({ val, lbl }) => (
              <div key={lbl} style={{
                background: "rgba(212,175,55,0.04)",
                border: "1px solid rgba(212,175,55,0.1)",
                borderRadius: 8,
                padding: "10px 12px",
                textAlign: "center",
              }}>
                <div style={{
                  fontSize: 15, fontWeight: 600,
                  fontFamily: "'Playfair Display', Georgia, serif",
                  background: "linear-gradient(135deg, #D4AF37, #B8953D)",
                  WebkitBackgroundClip: "text",
                  WebkitTextFillColor: "transparent",
                  marginBottom: 3,
                }}>
                  {val}
                </div>
                <div style={{ fontSize: 9, fontFamily: "monospace", letterSpacing: "0.14em", textTransform: "uppercase", color: "rgba(180,196,216,0.3)" }}>
                  {lbl}
                </div>
              </div>
            ))}
          </div>
        </div>

        {/* Footer */}
        <div style={{ position: "relative", zIndex: 1 }}>
          <p style={{ fontSize: 10, fontFamily: "monospace", letterSpacing: "0.15em", textTransform: "uppercase", color: "rgba(212,175,55,0.2)", margin: 0 }}>
            © 2026 WatchReg — Accès restreint
          </p>
        </div>
      </div>

      {/* ── Panneau droit — Formulaire ──────────────────────────────────────── */}
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 24, background: "var(--wr-bg)" }}>
        <div style={{ width: "100%", maxWidth: 360 }}>

          {/* Logo mobile */}
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 32 }} className="lg:hidden">
            <img src={LOGO_LG} alt="WatchReg" style={{ height: 52, width: "auto" }} />
          </div>

          {/* Icône + titre */}
          <div style={{ marginBottom: 28 }}>
            <div style={{
              width: 44, height: 44,
              borderRadius: 12,
              background: step === "mfa" ? "rgba(139,92,246,0.12)" : "rgba(212,175,55,0.1)",
              border: step === "mfa" ? "1px solid rgba(139,92,246,0.25)" : "1px solid rgba(212,175,55,0.25)",
              display: "flex", alignItems: "center", justifyContent: "center",
              marginBottom: 16,
            }}>
              {step === "mfa"
                ? <Smartphone size={20} style={{ color: "#A78BFA" }} />
                : <Lock size={20} style={{ color: "#D4AF37" }} />
              }
            </div>
            <h2 style={{
              fontSize: 20,
              fontWeight: 400,
              fontFamily: "var(--wr-font-serif)",
              color: "var(--wr-text-1)",
              letterSpacing: "-0.3px",
              margin: "0 0 4px",
            }}>
              {step === "mfa" ? t.auth.mfaTitle : t.auth.loginTitle}
            </h2>
            <p style={{ fontSize: 12, color: "var(--wr-text-3)", fontFamily: "monospace", margin: 0 }}>
              {step === "mfa" ? t.auth.mfaSubtitle : t.auth.restrictedAccess}
            </p>
          </div>

          {/* Formulaire credentials */}
          {step === "credentials" && (
            <form onSubmit={handleCredentials} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div>
                <label style={{ display: "block", fontSize: 10, fontFamily: "monospace", letterSpacing: "0.15em", textTransform: "uppercase", color: "rgba(212,175,55,0.5)", marginBottom: 8 }}>
                  {t.auth.email}
                </label>
                <input
                  type="email" value={email} required
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEmail(e.target.value)}
                  placeholder="analyste@institution.ma"
                  style={inputStyle}
                  onFocus={e => (e.target.style.borderColor = "var(--wr-accent-border)")}
                  onBlur={e => (e.target.style.borderColor = "var(--wr-border2)")}
                />
              </div>

              <div>
                <label style={{ display: "block", fontSize: 10, fontFamily: "monospace", letterSpacing: "0.15em", textTransform: "uppercase", color: "rgba(212,175,55,0.5)", marginBottom: 8 }}>
                  {t.auth.password}
                </label>
                <div style={{ position: "relative" }}>
                  <input
                    type={showPwd ? "text" : "password"}
                    value={password} required
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPassword(e.target.value)}
                    placeholder="••••••••"
                    style={{ ...inputStyle, paddingRight: 40 }}
                    onFocus={e => (e.target.style.borderColor = "rgba(212,175,55,0.35)")}
                    onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.1)")}
                  />
                  <button type="button" onClick={() => setShowPwd(!showPwd)} style={{
                    position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)",
                    background: "none", border: "none", cursor: "pointer", color: "var(--wr-text-3)", padding: 0,
                  }}>
                    {showPwd ? <EyeOff size={14} /> : <Eye size={14} />}
                  </button>
                </div>
              </div>

              {error && (
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.2)", borderRadius: 8 }}>
                  <AlertCircle size={13} style={{ color: "#F87171", flexShrink: 0 }} />
                  <span style={{ fontSize: 12, color: "#F87171", fontFamily: "monospace" }}>{error}</span>
                </div>
              )}

              <button type="submit" disabled={loading} style={{
                width: "100%",
                padding: "11px 16px",
                background: loading ? "rgba(212,175,55,0.3)" : "linear-gradient(135deg, #C9A227 0%, #B08020 50%, #C9A227 100%)",
                border: "none",
                borderRadius: 8,
                color: "#1A2B4B",
                fontSize: 13,
                fontWeight: 600,
                fontFamily: "'Playfair Display', Georgia, serif",
                letterSpacing: "0.03em",
                cursor: loading ? "not-allowed" : "pointer",
                transition: "all 0.2s",
                boxShadow: loading ? "none" : "0 4px 20px rgba(184,130,32,0.2)",
              }}>
                {loading ? t.common.loading : t.auth.login}
              </button>

              <div style={{ textAlign: "center" }}>
                <a href="/reset-password" style={{ fontSize: 11, fontFamily: "monospace", color: "var(--wr-text-3)", textDecoration: "none" }}
                  onMouseEnter={e => ((e.target as HTMLElement).style.color = "var(--wr-accent)")}
                  onMouseLeave={e => ((e.target as HTMLElement).style.color = "var(--wr-text-3)")}
                >
                  {t.auth.forgotPassword}
                </a>
              </div>
            </form>
          )}

          {/* Formulaire MFA */}
          {step === "mfa" && (
            <form onSubmit={handleMfa} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ padding: "10px 14px", background: "rgba(139,92,246,0.08)", border: "1px solid rgba(139,92,246,0.2)", borderRadius: 8, textAlign: "center" }}>
                <p style={{ fontSize: 11, fontFamily: "monospace", color: "#A78BFA", margin: 0 }}>
                  Google Authenticator · Authy · Bitwarden
                </p>
              </div>

              <div>
                <label style={{ display: "block", fontSize: 10, fontFamily: "monospace", letterSpacing: "0.15em", textTransform: "uppercase", color: "rgba(212,175,55,0.5)", marginBottom: 8 }}>
                  {t.auth.mfaCode}
                </label>
                <input
                  type="text" value={mfaCode} required autoFocus maxLength={8}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setMfaCode(e.target.value.replace(/\D/g,"").slice(0,6))}
                  placeholder="000000"
                  style={{ ...inputStyle, textAlign: "center", fontSize: 20, letterSpacing: "0.5em" }}
                  onFocus={e => (e.target.style.borderColor = "rgba(139,92,246,0.4)")}
                  onBlur={e => (e.target.style.borderColor = "var(--wr-border2)")}
                />
              </div>

              {error && (
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.2)", borderRadius: 8 }}>
                  <AlertCircle size={13} style={{ color: "#F87171", flexShrink: 0 }} />
                  <span style={{ fontSize: 12, color: "#F87171", fontFamily: "monospace" }}>{error}</span>
                </div>
              )}

              <button type="submit" disabled={loading || mfaCode.length < 6} style={{
                width: "100%", padding: "11px 16px",
                background: "linear-gradient(135deg, #7C3AED, #6D28D9)",
                border: "none", borderRadius: 8,
                color: "#F5F3FF", fontSize: 13, fontWeight: 600,
                fontFamily: "'Playfair Display', Georgia, serif",
                cursor: (loading || mfaCode.length < 6) ? "not-allowed" : "pointer",
                opacity: (loading || mfaCode.length < 6) ? 0.5 : 1,
                transition: "all 0.2s",
              }}>
                {loading ? t.common.loading : t.auth.mfaVerify}
              </button>

              <button type="button" onClick={() => { setStep("credentials"); setError(null); setMfaCode(""); }} style={{
                background: "none", border: "none", cursor: "pointer",
                fontSize: 11, fontFamily: "monospace", color: "var(--wr-text-3)",
                padding: "4px 0",
              }}>
                {t.auth.backToLogin}
              </button>
            </form>
          )}

          <p style={{ textAlign: "center", fontSize: 10, fontFamily: "monospace", letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--wr-text-4)", marginTop: 24 }}>
            WatchReg · Système de conformité réglementaire
          </p>
        </div>
      </div>
    </div>
  );
}
