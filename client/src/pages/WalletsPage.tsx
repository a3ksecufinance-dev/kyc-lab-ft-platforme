...
export function WalletsPage() {
  ...
  const COLUMNS: Column<WalletRow>[] = [
    {
      key: "id", header: "Wallet ID", width: "w-44",
    ...
    {
      key: "balance", header: "Solde", width: "w-36",
      render: (r) => (
      ...
      render: (r) => (
        r.isDormant
          ...
          : r.isActive
            ...
            : <span style={{ fontSize: 10, fontFamily: C.mono, color: C.red, background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.2)", borderRadius: 5, padding: "2px 7px" }}>SUSPENDU</span>
      ),
    },
    {
  ...
  return (
    <AppLayout>
      <div style={{ padding: "24px 28px", maxWidth: 1280 }}>
        ...
        <div style={{ marginBottom: 24, display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
          <div>
            <h1 style={{ fontSize: 18, fontWeight: 600, color: C.text1, margin: "0 0 4px", display: "flex", alignItems: "center", gap: 8 }}>
              <Wallet size={18} style={{ color: C.gold }} /> Wallets
            </h1>
            <p style={{ fontSize: 12, color: C.text3, margin: 0 }}>
              Gestion des wallets, tiers KYC et imports — {flags.institutionName}
            </p>
          </div>
          {isSupervisor && (
        ...
        {stats && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 24 }}>
            {[
              ...
              { label: "Volume 30j (MAD)", value: `${formatNumber(stats.volume30d ?? 0)}`, icon: <TrendingUp size={14} />,   color: C.green },
            ].map(kpi => (
        ...
        {selectedWallet && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
            <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 28, width: 380 }}>
              <h2 style={{ fontSize: 15, fontWeight: 600, color: C.text1, margin: "0 0 4px" }}>Promotion de tier</h2>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setSelectedWallet(null)}
                  style={{ flex: 1, fontSize: 12, padding: "8px 0", borderRadius: 8, border: `1px solid ${C.border2}`, background: "transparent", color: C.text3, cursor: "pointer" }}>Annuler</button>
        ...
        {suspendTarget && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
            <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 24, width: 380 }}>
              <h3 style={{ fontSize: 13, fontWeight: 600, fontFamily: C.mono, color: C.red, margin: "0 0 8px", display: "flex", alignItems: "center", gap: 6 }}>
                <Ban size={14} /> Suspendre le wallet
              </h3>
              <p style={{ fontSize: 11, fontFamily: C.mono, color: C.text3, margin: "0 0 16px" }}>{suspendTarget.walletId}</p>
              <div style={{ marginBottom: 16 }}>
                <label style={LABEL_S}>Motif de suspension *</label>
                <textarea value={suspendReason} onChange={e => setSuspendReason(e.target.value)} rows={3}
                  placeholder="Soupçon de fraude, gel des avoirs, plainte client…"
                  style={{ width: "100%", fontSize: 12, padding: "7px 10px", borderRadius: 7, border: `1px solid ${C.border2}`, background: "var(--wr-bg)", color: C.text1, outline: "none", resize: "vertical", fontFamily: "inherit", boxSizing: "border-box" }} />
              </div>
              {suspendMutation.error && <p style={{ fontSize: 11, color: C.red, margin: "0 0 8px" }}>{suspendMutation.error.message}</p>}
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => setSuspendTarget(null)} style={{ flex: 1, padding: "7px 0", fontSize: 12, fontFamily: C.mono, border: `1px solid ${C.border2}`, borderRadius: 7, color: C.text2, background: "transparent", cursor: "pointer" }}>Annuler</button>
                <button disabled={suspendReason.trim().length < 5 || suspendMutation.isPending}
                  ...
                  {suspendMutation.isPending ? "…" : "Confirmer"}
                </button>
              </div>
            </div>
          </div>
        ...
        {showBulkImport && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
            <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 28, width: 620, maxHeight: "88vh", overflowY: "auto" as const }}>

              {/* Header */}
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                <h3 style={{ fontSize: 15, fontWeight: 600, color: C.text1, margin: 0, display: "flex", alignItems: "center", gap: 8 }}>
                  <Upload size={15} style={{ color: C.gold }} /> Import global de transactions
                </h3>
                <button onClick={() => setShowBulkImport(false)} style={{ background: "none", border: "none", color: C.text3, cursor: "pointer" }}><X size={16} /></button>
              </div>
              <p style={{ fontSize: 12, color: C.text3, margin: "0 0 20px" }}>
                Importez un fichier CSV complet avec toutes les transactions. Le système regroupe automatiquement par numéro de téléphone et crée les wallets manquants.
              </p>
              ...
              <div style={{ background: C.hover, borderRadius: 8, padding: "10px 14px", marginBottom: 18, fontSize: 11, fontFamily: C.mono, color: C.text3, lineHeight: 1.8 }}>
                <div style={{ color: C.gold, marginBottom: 4, fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase" as const }}>Colonnes requises</div>
              ...
              >
                ...
                {bulkContent
                  ? <p style={{ fontSize: 12, color: C.green, fontFamily: C.mono, margin: 0 }}>{bulkFileName} — {bulkContent.split("\n").length - 1} lignes de données</p>
                  : <>
                      <p style={{ fontSize: 13, color: C.text2, margin: "0 0 4px" }}>Déposer le fichier CSV ici</p>
                      <p style={{ fontSize: 11, color: C.text3, margin: 0 }}>ou cliquer pour parcourir — CSV / MT940 / TXT</p>
                    </>}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
                <div>
                  <label style={LABEL_S}>Provider (détecté auto)</label>
              ...
              {bulkReport && (
                <div style={{ background: C.hover, borderRadius: 10, padding: 16, marginBottom: 16 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 8, marginBottom: 14 }}>
                    {[
                      { label: "Lignes", val: bulkReport.totalRows,       color: C.text2 },
                      { label: "Wallets trouvés",  val: bulkReport.walletsFound,   color: C.blue  },
                      { label: "Wallets créés",    val: bulkReport.walletsCreated, color: C.green },
                      { label: "Ignorés",          val: bulkReport.walletsSkipped, color: C.amber },
                      { label: "Tx insérées",      val: bulkReport.totalInserted,  color: C.gold  },
                    ].map(s => (
                  ...
                  {bulkReport.groups.length > 0 && (
                    <>
                      <div style={{ fontSize: 10, fontFamily: C.mono, color: C.text3, textTransform: "uppercase" as const, letterSpacing: "0.1em", marginBottom: 8 }}>
                        Détail par wallet ({bulkReport.groups.length})
                      </div>
                          ...
                          <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 10px", background: "rgba(255,255,255,0.03)", borderRadius: 6, border: `1px solid ${C.border}` }}>
                            <div style={{ width: 56, flexShrink: 0 }}>
                              <span style={{
                                ...
                                {g.action === "found" ? "TROUVÉ" : g.action === "created" ? "CRÉÉ" : "IGNORÉ"}
                              </span>
                            </div>
                  ...
                  {bulkReport.errors.length > 0 && (
                    <details style={{ marginTop: 10 }}>
                      <summary style={{ fontSize: 10, fontFamily: C.mono, color: C.red, cursor: "pointer" }}>{bulkReport.errors.length} erreur(s) de parsing</summary>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => setShowBulkImport(false)} style={{ flex: 1, padding: "9px 0", fontSize: 12, fontFamily: C.mono, border: `1px solid ${C.border2}`, borderRadius: 8, color: C.text2, background: "transparent", cursor: "pointer" }}>Fermer</button>
                <button
                  ...
                  {bulkImportMutation.isPending ? "Analyse en cours…" : bulkDryRun ? "Analyser le fichier (dry-run)" : "Importer toutes les transactions"}
                </button>
              </div>
            </div>
          </div>
        ...
        {importTarget && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
            <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 26, width: 560, maxHeight: "85vh", overflowY: "auto" as const }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                <h3 style={{ fontSize: 14, fontWeight: 600, color: C.text1, margin: 0, display: "flex", alignItems: "center", gap: 7 }}>
                  <Upload size={14} style={{ color: C.gold }} /> Import transactions
                </h3>
                <button onClick={() => setImportTarget(null)} style={{ background: "none", border: "none", color: C.text3, cursor: "pointer" }}><X size={16} /></button>
              </div>
              <p style={{ fontSize: 11, fontFamily: C.mono, color: C.text3, margin: "0 0 16px" }}>
                Wallet : <span style={{ color: C.blue }}>{importTarget.walletId}</span> — {importTarget.provider}
              </p>
              ...
              >
                ...
                {importContent
                  ...
                  : <p style={{ fontSize: 12, color: C.text3, margin: 0 }}>Déposer un fichier CSV / MT940 ou cliquer pour parcourir</p>}
              ...
              <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: C.text2, cursor: "pointer", marginBottom: 16 }}>
                ...
                Mode simulation (dry-run) — analyser sans insérer
              </label>

              {/* Rapport */}
              {importReport && (
                <div style={{ background: C.hover, borderRadius: 8, padding: 14, marginBottom: 16 }}>
                  <div style={{ fontSize: 10, fontFamily: C.mono, letterSpacing: "0.1em", textTransform: "uppercase" as const, color: C.text3, marginBottom: 10 }}>Rapport d'import</div>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 12 }}>
                    {[
                      { label: "Détectées",  val: importReport.total,      color: C.text2 },
                      { label: "Parsées",    val: importReport.parsed,     color: C.blue  },
                      { label: "Insertées",  val: importReport.inserted,   color: C.green },
                      { label: "Doublons",   val: importReport.duplicates, color: C.amber },
                    ].map(s => (
                      <div key={s.label} style={{ textAlign: "center" as const }}>
                  ...
                  {importReport.sampleRows.length > 0 && (
                    <>
                      <div style={{ fontSize: 10, fontFamily: C.mono, color: C.text3, marginBottom: 6 }}>Aperçu</div>
                  ...
                  {importReport.errors.length > 0 && (
                    <details style={{ marginTop: 8 }}>
                      <summary style={{ fontSize: 10, fontFamily: C.mono, color: C.red, cursor: "pointer" }}>{importReport.errors.length} erreur(s)</summary>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => setImportTarget(null)} style={{ flex: 1, padding: "8px 0", fontSize: 12, fontFamily: C.mono, border: `1px solid ${C.border2}`, borderRadius: 7, color: C.text2, background: "transparent", cursor: "pointer" }}>Fermer</button>
                <button
                  ...
                  {importMutation.isPending ? "Import en cours…" : importDryRun ? "Analyser (dry-run)" : "Importer les transactions"}
                </button>
              </div>
            </div>
          </div>
        ...
        {detailId && walletDetail && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
            <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 24, width: 560, maxHeight: "85vh", overflowY: "auto" as const }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <button onClick={() => setDetailId(null)} style={{ background: "none", border: "none", color: C.text3, cursor: "pointer" }}><X size={16} /></button>
                </div>
              </div>
              ...
              <div style={{ display: "flex", gap: 2, marginBottom: 18, borderBottom: `1px solid ${C.border}`, paddingBottom: 0 }}>
                {([
                  ...
                  { id: "usage" as const,        label: "Limites" },
                  { id: "transactions" as const, label: "Transactions" },
                  { id: "history" as const,      label: "Tiers KYC" },
                ] as const).map(tab => (
              ...
              {detailTab === "info" && (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
                  {[
                    ...
                    { label: "Statut",     value: walletDetail.isDormant ? "DORMANT" : walletDetail.isActive ? "ACTIF" : "SUSPENDU" },
                    { label: "Tier KYC",   value: walletDetail.kycTier },
                    { label: "Provider",   value: walletDetail.provider },
                    { label: "Téléphone",  value: (walletDetail.phoneNumber ?? "—") as string },
              ...
              {detailTab === "usage" && (
                <div>
                  {usageData ? (
                    <>
                      <div style={{ marginBottom: 16 }}>
                        <UsageBar label="Utilisation journalière"  used={usageData.dailyUsed}   limit={usageData.dailyLimit}   currency={usageData.currency} />
                        <UsageBar label="Utilisation mensuelle"    used={usageData.monthlyUsed} limit={usageData.monthlyLimit} currency={usageData.currency} />
                      </div>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                        <div style={{ background: C.hover, borderRadius: 6, padding: "10px 12px" }}>
                          <div style={{ fontSize: 9, fontFamily: C.mono, color: C.text3, textTransform: "uppercase" as const, letterSpacing: "0.1em", marginBottom: 4 }}>Solde actuel</div>
                        ...
                        <div style={{ background: C.hover, borderRadius: 6, padding: "10px 12px" }}>
                          <div style={{ fontSize: 9, fontFamily: C.mono, color: C.text3, textTransform: "uppercase" as const, letterSpacing: "0.1em", marginBottom: 4 }}>Tier KYC</div>
                          <TierBadge tier={usageData.tier} />
                        </div>
                      </div>
                    </>
                  ) : (
                    <p style={{ fontSize: 12, color: C.text3, fontFamily: C.mono }}>Chargement…</p>
                  )}
                </div>
              )}
              ...
              {detailTab === "transactions" && (
                <div>
                  {!txData ? (
                    ...
                    <p style={{ fontSize: 12, color: C.text3, fontFamily: C.mono }}>Aucune transaction liée à ce wallet.</p>
                  ) : (
                    <>
